...
最近了解了一下微前端的概念，刚好有分享会，顺便和大家分享一下了解到的相关概念，可以稍微简单地拓展一下前端的新概念
...
从上图可以明确地看到，微前端的主要布局可以分为三个部分：Header头部(可选，但一般都会有)，菜单(可选)，以及子应用区域。
...
其实确实有存在iframe的解决方案，那就是wujie。由于本次分享以qiankun为例，就不详细说wujie的解决这些问题的思路了，有兴趣的小伙伴可以自行去了解一下。
...
* 主应用的hash路由是否会和vue等框架的子应用的hash路由冲突，有待考究

## JS沙箱

JS沙箱通过限制代码的访问权限、提供隔离的执行环境、使用安全策略和沙箱沙盒技术等手段来实现。常见的JavaScript沙箱技术包括沙盒环境、Web Worker、iframe、JavaScript虚拟机等。这些技术通过限制代码的执行权限、提供独立的运行环境、隔离全局上下文等方式来确保代码的安全执行。
...
我们需要在子应用的JS代码，调起一个提示窗挂在主容器全局的body下（因为子应用的布局未必是铺满整个主容器的），但同时要使用子应用内部的样式，那么这时候就会有问题了，css被彻底隔离在shadow DOM中，外部就读取不到了
...
| 使用场景         | 一包多用(lab,cs,bp)，按需引入子模块，灵活打包，有效降低服务器的存储压力 | 大型工程解耦维护或改造                                                             |
...
* 继续研究思考大型前端项目的工程化问题，思考工业软件的web架构；
* 研究端到端的Electron的开发，构建，迭代，灰度机制；