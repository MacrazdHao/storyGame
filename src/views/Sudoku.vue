...
export default {
  ...
  methods: {
    ...
    getUniqueMatrix(
      ...
      if (numRowIndex === this.MatrixWidth) return false
      ...
      const minCombLength = Math.max(
        0,
        this.MatrixWidth - (remainNum - remainDiffNum)
      )
      const combLengthArr = new Array(combLength - minCombLength + 1)
        .fill(0)
        .map((item, index) => minCombLength + index)
        .sort(() => {
      ...
      for (let cn = 0; cn < combLengthArr.length; cn++) {
        ...
        for (let i = 0; i < comb.length; i++) {
          ...
          // 该组合符合条件
          if (isOk) {
            const res = this.getUniqueMatrix(
              rows,
              cols,
              boxs,
              numRowIndex + 1,
    ...
    getUncrackMatrix() {
      if (this.gettingUncrackMatrix) return
      this.gettingUncrackMatrix = true
      let matrix = false
      while (!matrix) {
        this.getMatrix()
      ...
      this.gettingUncrackMatrix = false
    },
    getDiffNums(a, b, c) {
      // a∪b∪c
